...
import {addIndexAd, queryIndexAd, removeIndexAd, updateIndexAd} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryGoodsCategory} from '@/pages/AutoGenPage//GoodsCategoryList/service';import {queryGoods} from '@/pages/AutoGenPage//GoodsList/service';
...
const TableList = () => {
  ...
  try {
    await updateIndexAd(value, current_id);
    hide();
                              ...
                              rules: [
                                {
                                  ...
                                  message: 'goods为必填项',
                                    ...
                                    renderFormItem: (item, {value, onChange}) => {
            const children = goodsForeignKeyList.map((item) => {
              return <Option key={item.id} value={item.id}>{item.name}</Option>;
            });
            return <Select
              placeholder="请选择goods"
              onChange={onChange}
            >
              {children}
            </Select>;
  ...
  const [categoryForeignKeyList, setCategoryForeignKeyList] = useState([]);
      ...
      useEffect(() => {
        queryGoods().then(value => {
          setGoodsForeignKeyList(value.data);
        });
      }, []);
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, {selectedRows}) => [
          ...
          <Input.Search style={{marginRight: 20}} placeholder="搜索首页广告 " onSearch={value => {
            setParamState({
              search: value,
            });
        ...
        request={(params, sorter, filter) => queryIndexAd({...params, sorter, filter})}
