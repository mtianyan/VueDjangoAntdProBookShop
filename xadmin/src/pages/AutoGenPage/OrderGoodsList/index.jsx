...
import {addOrderGoods, queryOrderGoods, removeOrderGoods, updateOrderGoods} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryOrderInfo} from '@/pages/AutoGenPage//OrderInfoList/service';import {queryGoods} from '@/pages/AutoGenPage//GoodsList/service';
...
const TableList = () => {
  ...
  const base_columns = [{
            ...
            const children = orderForeignKeyList.map((item) => {
              return <Option key={item.id} value={item.id}>{item.order_sn}</Option>;
            });
            return <Select
              placeholder="请选择订单信息"
              onChange={onChange}
  ...
  const [orderForeignKeyList, setOrderForeignKeyList] = useState([]);
      useEffect(() => {
        queryOrderInfo().then(value => {
          setOrderForeignKeyList(value.data);
        });
