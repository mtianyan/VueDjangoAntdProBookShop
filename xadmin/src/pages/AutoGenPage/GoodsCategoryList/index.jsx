...
import {addGoodsCategory, queryGoodsCategory, removeGoodsCategory, updateGoodsCategory} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
// import {queryGoodsCategory} from '@/pages/AutoGenPage//GoodsCategoryList/service';
...
const TableList = () => {
  ...
  const base_columns = [{
            ...
            const children = parent_categoryForeignKeyList.map((item) => {
              return <Option key={item.id} value={item.id}>{item.name}</Option>;
            });
            return <Select
              placeholder="请选择父类目级别"
              onChange={onChange}
            >
              {children}
            </Select>;
          ...
          const is_value = form.getFieldValue('is_tab');
          if (type === "form" && !is_value) {
            form.setFieldsValue({'is_tab': false});
          }
          return <Switch checked={is_value} onClick={(checked) => {
            form.setFieldsValue({'is_tab': checked});
          }} />;
        },
                                },{
              title: '添加时间',
  ...
  const [parent_categoryForeignKeyList, setParent_categoryForeignKeyList] = useState([]);
      useEffect(() => {
        queryGoodsCategory().then(value => {
          setParent_categoryForeignKeyList(value.data);
        });
      }, []);
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, {selectedRows}) => [
          ...
          <Input.Search style={{marginRight: 20}} placeholder="搜索商品类别 " onSearch={value => {
            setParamState({
              search: value,
            });
