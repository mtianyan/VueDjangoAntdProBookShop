...
import { addGoodsImage, queryGoodsImage, removeGoodsImage, updateGoodsImage, queryGoodsImageVerboseName, queryGoodsImageListDisplay, queryGoodsImageDisplayOrder} from './service';
import UpdateForm from './components/UpdateForm';
import UploadAvatar from '@/components/UploadAvatar';
import {queryGoods, queryGoodsVerboseName} from '@/pages/AutoGenPage/GoodsList/service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateGoodsImage(value, current_id);
      hide();
...
renderFormItem: (_, {type, defaultRender, ...rest}, form) => {
  ...
  useEffect(() => {
    queryGoodsImageDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryGoodsImageListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryGoodsImage, table_columns, '商品轮播-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索商品轮播" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '商品轮播-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryGoodsImage({ ...params, sorter, filter })}
