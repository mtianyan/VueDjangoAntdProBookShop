...
import { addUserFav, queryUserFav, removeUserFav, updateUserFav, queryUserFavVerboseName, queryUserFavListDisplay, queryUserFavDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateUserFav(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryUserFavDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryUserFavListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryUserFav, table_columns, '用户收藏-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '用户收藏-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryUserFav({ ...params, sorter, filter })}
