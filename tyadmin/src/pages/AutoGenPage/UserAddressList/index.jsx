...
import { addUserAddress, queryUserAddress, removeUserAddress, updateUserAddress, queryUserAddressVerboseName, queryUserAddressListDisplay, queryUserAddressDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateUserAddress(value, current_id);
      hide();
  ...
  useEffect(() => {
    queryUserAddressDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryUserAddressListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryUserAddress, table_columns, '收货地址-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索收货地址" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '收货地址-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryUserAddress({ ...params, sorter, filter })}
