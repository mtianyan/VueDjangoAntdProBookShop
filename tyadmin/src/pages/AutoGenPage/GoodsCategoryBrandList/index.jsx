...
import { addGoodsCategoryBrand, queryGoodsCategoryBrand, removeGoodsCategoryBrand, updateGoodsCategoryBrand, queryGoodsCategoryBrandVerboseName, queryGoodsCategoryBrandListDisplay, queryGoodsCategoryBrandDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleAdd = async fields => {
    ...
    try {
      await addGoodsCategoryBrand({ ...fields });
      hide();
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateGoodsCategoryBrand(value, current_id);
      hide();
  ...
  const handleRemove = async selectedRows => {
    ...
    try {
      ...
      await removeGoodsCategoryBrand(ids);
      hide();
...
renderFormItem: (_, {type, defaultRender, ...rest}, form) => {
  ...
  useEffect(() => {
    queryGoodsCategoryBrandDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryGoodsCategoryBrandListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryGoodsCategoryBrand, table_columns, '宣传品牌-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索宣传品牌" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '宣传品牌-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryGoodsCategoryBrand({ ...params, sorter, filter })}
