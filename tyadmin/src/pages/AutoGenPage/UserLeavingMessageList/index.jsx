...
import { addUserLeavingMessage, queryUserLeavingMessage, removeUserLeavingMessage, updateUserLeavingMessage, queryUserLeavingMessageVerboseName, queryUserLeavingMessageListDisplay, queryUserLeavingMessageDisplayOrder} from './service';
...
const TableList = () => {
  ...
  const handleAdd = async fields => {
    ...
    try {
      await addUserLeavingMessage({ ...fields });
      hide();
  ...
  const handleUpdate = async (value, current_id) => {
    ...
    try {
      await updateUserLeavingMessage(value, current_id);
      hide();
  ...
  const handleRemove = async selectedRows => {
    ...
    try {
      ...
      await removeUserLeavingMessage(ids);
      hide();
  ...
  const base_columns = [{
        ...
        hideInSearch: true,
                        ...
                        },
                      {
                             ...
                             valueEnum:{"1":"留言","2":"投诉","3":"询问","4":"售后","5":"求购"}
                        },
                      {
                             title: '主题',
  ...
  useEffect(() => {
    queryUserLeavingMessageDisplayOrder().then(r => {
  ...
  useEffect(() => {
    queryUserLeavingMessageListDisplay().then(value => {
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, { selectedRows }) => [
          ...
          <Button type="primary" onClick={() => exportExcelAll(paramState, queryUserLeavingMessage, table_columns, '用户留言-All')}>
            <ExportOutlined /> 导出全部
          </Button>,
          <Input.Search style={{ marginRight: 20 }} placeholder="搜索用户留言" onSearch={value => {
            setParamState({
              search: value,
            });
                ...
                <Menu
                  onClick={async e => {
                    if (e.key === 'remove') {
                      ...
                      exportExcelCurrent(selectedRows, table_columns, '用户留言-select')
                    }
                  }}
                  selectedKeys={[]}
                >
        ...
        request={(params, sorter, filter) => queryUserLeavingMessage({ ...params, sorter, filter })}
