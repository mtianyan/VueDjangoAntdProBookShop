...
import {addHotSearchWords, queryHotSearchWords, removeHotSearchWords, updateHotSearchWords} from './service';
...
const TableList = () => {
  ...
  try {
    await addHotSearchWords({...fields});
    hide();
  ...
  try {
    await updateHotSearchWords(value, current_id);
    hide();
  ...
  try {
    ...
    await removeHotSearchWords(ids);
    hide();
    ...
    <PageHeaderWrapper>
      <ProTable
        ...
        toolBarRender={(action, {selectedRows}) => [
          ...
          <Input.Search style={{marginRight: 20}} placeholder="搜索热搜排行 " onSearch={value => {
            setParamState({
              search: value,
            });
        ...
        request={(params, sorter, filter) => queryHotSearchWords({...params, sorter, filter})}
